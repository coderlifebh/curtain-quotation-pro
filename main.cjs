...
ipcMain.handle('updater:check-for-updates', async () => {
  try {
    ...
    // In development mode, perform manual update check
    if (process.env.NODE_ENV === 'development' || !app.isPackaged) {
      try {
        ...
        const githubUrl = 'https://api.github.com/repos/coderlifebh/curtain-quotation-pro/releases/latest';
        
        return new Promise((resolve) => {
          const req = https.get(githubUrl, { 
            headers: { 'User-Agent': 'Curtain-Quotation-Pro' }
          }, (res) => {
            let data = '';
              ...
              try {
                if (res.statusCode === 404) {
                  resolve({ 
                    ...
                    hasUpdate: false,
                    currentVersion,
                    note: 'No releases found on GitHub'
                  });
                  return;
                }
                ...
                if (res.statusCode !== 200) {
                  resolve({ 
                    ...
                    hasUpdate: false,
                    currentVersion,
                    note: 'Unable to check GitHub releases'
                  });
                  return;
                }
                ...
                if (latestVersion && latestVersion !== currentVersion) {
                  resolve({ 
                    ...
                    currentVersion,
                    latestVersion,
                    note: 'New version found on GitHub'
                  });
                } else {
                  resolve({ 
                    message: 'No updates available', 
                ...
                resolve({ 
                  ...
                  hasUpdate: false,
                  currentVersion,
                  note: 'Error checking for updates'
                });
              }
            });
          });
          ...
          req.on('error', (error) => {
            ...
            resolve({ 
              ...
              hasUpdate: false,
              currentVersion,
              note: 'Network error - unable to check for updates'
            });
          });
          ...
          req.setTimeout(10000, () => {
            ...
            resolve({ 
              ...
              hasUpdate: false,
              currentVersion,
              note: 'Request timeout - unable to check for updates'
            });
          });
        });
      } catch (error) {
    ...
    // In production mode, use the auto-updater
    autoUpdater.checkForUpdates();
    return { message: 'Checking for updates...', currentVersion };
  } catch (error) {
