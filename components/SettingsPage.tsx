...
const sharedInputClass = (isDarkTheme: boolean) => {
    return `w-full border rounded-md shadow-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${isDarkTheme
...
export const SettingsPage: React.FC<SettingsPageProps> = ({ settings, onSettingsChange, onExport, onImport, onReset, onResetColumnWidths, isDarkTheme = true }) => {
                ...
                return (
                    <div className="space-y-6">
                        <div className={`rounded-lg p-6 shadow-2xl ${isDarkTheme ? 'bg-slate-800 shadow-slate-950/50' : 'bg-white shadow-gray-200/50'}`}>
                            <h2 className={`text-xl font-bold mb-4 ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>Company Details</h2>
                            <p className={`text-sm mb-6 ${isDarkTheme ? 'text-slate-400' : 'text-gray-500'}`}>This information will be used as the default 'From' address on new quotations.</p>
                            <div className="space-y-4">
                                <div>
                                    <label htmlFor="companyName" className={`block text-sm font-medium mb-1 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>Company Name</label>
                                    <input
                                        id="companyName"
                                        type="text"
                                        ...
                                        className={sharedInputClass(isDarkTheme)}
                                        placeholder="Your Company Name"
                                    />
                                </div>
                                <div>
                                    <label htmlFor="companyAddress" className={`block text-sm font-medium mb-1 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>Address</label>
                                    <input
                                        id="companyAddress"
                                        type="text"
                        ...
                        <div className="${isDarkTheme ? `bg-slate-800 shadow-slate-950/50` : `bg-white shadow-gray-200/50`} shadow-2xl rounded-lg p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input id="defaultDiscount" type="number" value={localSettings.defaultDiscountPercent} onChange={e => handleNumericChange('defaultDiscountPercent', e.target.value)} className={sharedInputClass(isDarkTheme)} min="0" />
                                </div>
                                <div>
                                    <label htmlFor="defaultTax" className="block text-sm font-medium text-slate-300 mb-1">Default Tax (%)</label>
                                    <input id="defaultTax" type="number" value={localSettings.defaultTaxPercent} onChange={e => handleNumericChange('defaultTaxPercent', e.target.value)} className={sharedInputClass(isDarkTheme)} min="0" />
                                </div>
                                <div>
                                    <label htmlFor="defaultExpiration" className="block text-sm font-medium text-slate-300 mb-1">Default Expiration (Days)</label>
                                    <input
                                        id="defaultExpiration"
                                        type="number"
                                        value={localSettings.defaultExpirationDays}
                                        onChange={e => handleNumericChange('defaultExpirationDays', e.target.value)}
                                        className={sharedInputClass(isDarkTheme)}
                                        min="0"
                                        placeholder="Days until quotation expires"
                                    />
                                    <p className="text-xs text-slate-400 mt-1">Number of days from quotation date when it expires (0 = no expiration)</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="${isDarkTheme ? `bg-slate-800 shadow-slate-950/50` : `bg-white shadow-gray-200/50`} shadow-2xl rounded-lg p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    ...
                                    <input id="defaultItemWidth" type="number" value={localSettings.defaultItemWidth} onChange={e => handleNumericChange('defaultItemWidth', e.target.value)} className={sharedInputClass(isDarkTheme)} min="0" />
                                </div>
                                <div>
                                    <label htmlFor="defaultItemHeight" className="block text-sm font-medium text-slate-300 mb-1">Default Height (cm)</label>
                                    <input id="defaultItemHeight" type="number" value={localSettings.defaultItemHeight} onChange={e => handleNumericChange('defaultItemHeight', e.target.value)} className={sharedInputClass(isDarkTheme)} min="0" />
                                </div>
                                <div>
                                    <label htmlFor="defaultItemQuantity" className="block text-sm font-medium text-slate-300 mb-1">Default Quantity</label>
                                    <input id="defaultItemQuantity" type="number" value={localSettings.defaultItemQuantity} onChange={e => handleNumericChange('defaultItemQuantity', e.target.value)} className={sharedInputClass(isDarkTheme)} min="1" />
                                </div>
                            </div>
                        </div>
                        <div className="${isDarkTheme ? `bg-slate-800 shadow-slate-950/50` : `bg-white shadow-gray-200/50`} shadow-2xl rounded-lg p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input id="defaultTemplateFullnessFront" type="number" value={localSettings.defaultTemplateFullnessFront} onChange={e => handleNumericChange('defaultTemplateFullnessFront', e.target.value)} className={sharedInputClass(isDarkTheme)} min="0" step="0.1" />
                                </div>
                                <div>
                                    ...
                                    <input id="defaultTemplateFullnessBack" type="number" value={localSettings.defaultTemplateFullnessBack} onChange={e => handleNumericChange('defaultTemplateFullnessBack', e.target.value)} className={sharedInputClass(isDarkTheme)} min="0" step="0.1" />
                                </div>
                            </div>
                        </div>
                        <div className="${isDarkTheme ? `bg-slate-800 shadow-slate-950/50` : `bg-white shadow-gray-200/50`} shadow-2xl rounded-lg p-6">
                                ...
                                {localSettings.deliveryEstimateTiers.map((tier) => (
                                    <div key={tier.id} className="grid grid-cols-1 sm:grid-cols-8 gap-3 items-center">
                                        <div className="sm:col-span-2">
                                            ...
                                            <input id={`min-${tier.id}`} type="number" value={tier.minWindows} onChange={e => handleTierChange(tier.id, 'minWindows', e.target.value)} className={sharedInputClass(isDarkTheme)} placeholder="Min" />
                                        </div>
                                        <div className="sm:col-span-2">
                                            <label htmlFor={`max-${tier.id}`} className="sr-only">Max Windows</label>
                                            <input id={`max-${tier.id}`} type="number" value={tier.maxWindows} onChange={e => handleTierChange(tier.id, 'maxWindows', e.target.value)} className={sharedInputClass(isDarkTheme)} placeholder="Max" />
                                        </div>
                    ...
                    <div className="space-y-6">
                        <div className="${isDarkTheme ? `bg-slate-800 shadow-slate-950/50` : `bg-white shadow-gray-200/50`} shadow-2xl rounded-lg p-6">
                            ...
                            <div className="space-y-4">
                                ...
                                {updateInfo.latestVersion && updateInfo.latestVersion !== updateInfo.version && (
                                    <div className="bg-slate-700/50 p-4 rounded-lg">
                                        <h3 className="text-lg font-medium text-white mb-2">Latest Version</h3>
                                        <p className="text-green-400 text-lg font-mono">{updateInfo.latestVersion}</p>
                                    </div>
                                )}
                                ...
                                <div className="bg-slate-700/50 p-4 rounded-lg">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {!updateInfo.hasUpdate && !updateInfo.error && !updateInfo.isChecking && !updateInfo.hasChecked && (
                                            <div className="flex items-center gap-3">
                                        ...
                                        {!updateInfo.hasUpdate && !updateInfo.error && !updateInfo.isChecking && updateInfo.hasChecked && (
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div>
                                                    <span className="text-blue-400 font-medium">You're up to date</span>
                                                    {updateInfo.note && (
                                                        <p className="text-slate-300 text-sm mt-1">{updateInfo.note}</p>
                                                    )}
                                                </div>
                                            </div>
                                        )}
    ...
    return (
        <div className="space-y-8 max-w-5xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <h1 className={`text-3xl font-bold ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>Settings</h1>
                <button
                    onClick={handleSave}
                ...
                <nav className="-mb-px flex flex-wrap space-x-6 sm:space-x-8" aria-label="Tabs">
                    {TABS.map((tab) => (
                        <button
                            ...
                            className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors focus:outline-none ${activeTab === tab.id
                                    ? 'border-blue-500 text-blue-400'
                                    : `border-transparent ${isDarkTheme ? 'text-slate-400 hover:text-slate-200 hover:border-slate-500' : 'text-gray-500 hover:text-gray-700 hover:border-gray-300'}`
                                }`}
