...
const StatusUpdateModal: React.FC<{
    ...
    isDarkTheme?: boolean;
}> = ({ quote, onClose, onSave, isDarkTheme = true }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
            <div className={`rounded-lg shadow-xl w-full max-w-lg m-4 ${isDarkTheme ? 'bg-slate-800' : 'bg-white'}`}>
                <div className={`flex justify-between items-center p-4 border-b ${isDarkTheme ? 'border-slate-700' : 'border-gray-200'}`}>
                    <h3 className={`text-lg font-bold truncate ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>Update Status: "{quote.name}"</h3>
                    <button onClick={onClose} className={`${isDarkTheme ? 'text-slate-400 hover:text-white' : 'text-gray-400 hover:text-gray-600'}`}><CloseIcon className="w-6 h-6" /></button>
                </div>
                <div className="p-6 space-y-4">
                    <div>
                        <label htmlFor="status-select" className={`block text-sm font-medium mb-1 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>New Status</label>
                        <select
                            id="status-select"
                            value={newStatus}
                            onChange={(e) => setNewStatus(e.target.value as QuotationStatus)}
                            className={`w-full border rounded-md shadow-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${isDarkTheme
                    ...
                    <div>
                        <label htmlFor="status-notes" className={`block text-sm font-medium mb-1 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>Notes (Optional)</label>
                        <textarea
                            id="status-notes"
                            rows={3}
                            value={notes}
                    ...
                    <div className={`border-t pt-4 ${isDarkTheme ? 'border-slate-700' : 'border-gray-200'}`}>
                        <h4 className={`text-sm font-medium mb-2 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>Status History</h4>
                        {quote.statusHistory && quote.statusHistory.length > 0 ? (
                            <ul className="space-y-2 max-h-40 overflow-y-auto text-xs">
                                {[...quote.statusHistory].reverse().map(entry => (
                                    <li key={entry.date} className={`p-2 rounded-md ${isDarkTheme ? 'bg-slate-700/50' : 'bg-gray-100'}`}>
                                        <p className={`font-semibold ${isDarkTheme ? 'text-slate-200' : 'text-gray-800'}`}>
                                            <StatusBadge status={entry.status} /> on {new Date(entry.date).toLocaleString()}
                                        </p>
                                        {entry.notes && <p className={`mt-1 italic ${isDarkTheme ? 'text-slate-400' : 'text-gray-600'}`}>"{entry.notes}"</p>}
                                    </li>
                                ))}
                            </ul>
                        ) : (
                            <p className={`text-xs ${isDarkTheme ? 'text-slate-500' : 'text-gray-500'}`}>No history available.</p>
                        )}
                    </div>
                </div>
                <div className={`flex justify-end gap-2 p-4 border-t ${isDarkTheme ? 'bg-slate-800/50 border-slate-700' : 'bg-gray-50 border-gray-200'}`}>
                    <button onClick={onClose} className={`px-4 py-2 text-sm font-semibold rounded-lg shadow-md ${isDarkTheme ? 'bg-slate-600 text-white hover:bg-slate-700' : 'bg-gray-500 text-white hover:bg-gray-600'}`}>Cancel</button>
...
export const SavedQuotationsPage: React.FC<SavedQuotationsPageProps> = ({ quotations, onLoad, onDelete, onCopy, onPreview, onRestore, onDeletePermanently, onRestoreVersion, onPreviewVersion, clientFilter, onClearFilter, onStatusUpdate, isDarkTheme = true }) => {
        ...
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                {clientFilter ? (
                    <div className="flex items-center gap-4 min-w-0">
                        <h1 className={`text-3xl font-bold ${isDarkTheme ? 'text-white' : 'text-gray-900'} truncate`} title={clientFilter}><span className={`${isDarkTheme ? 'text-slate-400' : 'text-gray-500'} font-normal`}>Client:</span> {clientFilter}</h1>
                        <button onClick={onClearFilter} className="text-sm text-blue-500 hover:text-blue-600 whitespace-nowrap flex-shrink-0">(Show All)</button>
                    </div>
                ) : <h1 className={`text-3xl font-bold ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>Saved Quotations</h1>}

                <div className="flex items-center gap-2">
                    <div className="relative w-full md:max-w-xs">
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><SearchIcon className={`h-5 w-5 ${isDarkTheme ? 'text-slate-400' : 'text-gray-400'}`} /></div>
                        <input type="text" placeholder="Search by name, client, or QN..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full rounded-lg shadow-sm pl-10 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${isDarkTheme ? 'bg-slate-700/50 border border-slate-600 text-white placeholder-slate-400' : 'bg-white border border-gray-300 text-gray-900 placeholder-gray-500'}`} />
                    </div>
                    <button onClick={() => setTrashOpen(true)} className={`flex-shrink-0 relative px-4 py-2 text-sm text-white font-semibold rounded-lg shadow-md ${isDarkTheme ? 'bg-slate-600 hover:bg-slate-700' : 'bg-gray-500 hover:bg-gray-600'}`} title="Open Trash">
            ...
            {activeQuotations.length === 0 ? (
                <div className={`text-center py-16 rounded-lg shadow-xl ${isDarkTheme ? 'bg-slate-800' : 'bg-white'}`}>
                    <svg className={`mx-auto h-12 w-12 ${isDarkTheme ? 'text-slate-500' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z" /></svg>
                    <h3 className={`mt-2 text-lg font-medium ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>No saved quotations</h3>
                    <p className={`mt-1 text-sm ${isDarkTheme ? 'text-slate-400' : 'text-gray-500'}`}>Go to the 'Quotation' page to save your first one.</p>
                </div>
            ) : filteredAndSortedQuotations.length === 0 ? (
                <div className={`text-center py-16 rounded-lg shadow-xl ${isDarkTheme ? 'bg-slate-800' : 'bg-white'}`}>
                    <SearchIcon className={`mx-auto h-12 w-12 ${isDarkTheme ? 'text-slate-500' : 'text-gray-400'}`} />
                    <h3 className={`mt-2 text-lg font-medium ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>No Results Found</h3>
                    <p className={`mt-1 text-sm ${isDarkTheme ? 'text-slate-400' : 'text-gray-500'}`}>Your search for "{searchQuery}" did not match any quotations{clientFilter ? ` for ${clientFilter}` : ''}.</p>
                </div>
            ) : (
                        ...
                        <table className="min-w-full divide-y divide-slate-700">
                            ...
                            <tbody className={`${isDarkTheme ? 'bg-slate-800 divide-slate-700' : 'bg-white divide-gray-200'} divide-y`}>
                                {filteredAndSortedQuotations.map(q => (
                                    <tr key={q.id} className={`${isDarkTheme ? 'hover:bg-slate-700/40' : 'hover:bg-gray-50'} transition-colors`}>
                                        <td className={`px-4 py-3 whitespace-nowrap text-sm font-medium ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>{q.name}</td>
                                        <td className={`px-4 py-3 whitespace-nowrap text-sm ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>{q.quotationNumber}</td>
                                        <td className={`px-4 py-3 whitespace-nowrap text-sm ${isDarkTheme ? 'text-slate-400' : 'text-gray-500'}`}>{new Date(q.quotationDate + 'T00:00:00').toLocaleDateString()}</td>
                                        <td className={`px-4 py-3 whitespace-nowrap text-sm ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>{q.clientDetails.name || 'N/A'}</td>
                                        <td className={`px-4 py-3 whitespace-nowrap text-sm ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>
                                            <StatusBadge status={q.status} />
                                        </td>
                                        <td className={`px-4 py-3 whitespace-nowrap text-sm font-semibold ${isDarkTheme ? 'text-blue-400' : 'text-blue-600'}`}>{q.grandTotal.toLocaleString('en-US', { style: 'currency', currency: 'BHD' })}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-right space-x-1">
                                            <button onClick={() => handleOpenStatusModal(q)} className={`${isDarkTheme ? 'text-slate-400 hover:text-cyan-400' : 'text-gray-500 hover:text-cyan-500'} p-1`} title="Update Status"><StatusIcon className="w-5 h-5" /></button>
                                            {q.history && q.history.length > 0 && <button onClick={() => { setSelectedQuoteForHistory(q); setHistoryOpen(true); }} className={`${isDarkTheme ? 'text-slate-400 hover:text-yellow-400' : 'text-gray-500 hover:text-yellow-500'} p-1`} title="History"><HistoryIcon className="w-5 h-5" /></button>}
                                            <button onClick={() => onPreview(q.id)} className={`${isDarkTheme ? 'text-slate-400 hover:text-purple-400' : 'text-gray-500 hover:text-purple-500'} p-1`} title="Preview"><PreviewIcon className="w-5 h-5" /></button>
                                            <button onClick={() => onLoad(q.id)} className={`${isDarkTheme ? 'text-slate-400 hover:text-blue-400' : 'text-gray-500 hover:text-blue-500'} p-1`} title="Edit"><EditIcon className="w-5 h-5" /></button>
                                            <button onClick={() => onCopy(q.id)} className={`${isDarkTheme ? 'text-slate-400 hover:text-green-400' : 'text-gray-500 hover:text-green-500'} p-1`} title="Copy"><CopyIcon className="w-5 h-5" /></button>
                                            <button onClick={() => onDelete(q.id)} className={`${isDarkTheme ? 'text-slate-400 hover:text-red-500' : 'text-gray-500 hover:text-red-500'} p-1`} title="Move to Trash"><TrashIcon className="w-5 h-5" /></button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
            ...
            {isHistoryOpen && selectedQuoteForHistory && <HistoryModal quote={selectedQuoteForHistory} onClose={() => setHistoryOpen(false)} onRestoreVersion={onRestoreVersion} onPreviewVersion={onPreviewVersion} isDarkTheme={isDarkTheme} />}
            {isTrashOpen && <TrashModal trashed={trashedQuotations} onRestore={onRestore} onDeletePermanently={onDeletePermanently} onClose={() => setTrashOpen(false)} isDarkTheme={isDarkTheme} />}
            {isStatusModalOpen && selectedQuoteForStatus && <StatusUpdateModal quote={selectedQuoteForStatus} onClose={() => setStatusModalOpen(false)} onSave={onStatusUpdate} isDarkTheme={isDarkTheme} />}
        </div>
    );
};