...
export const ClientsPage: React.FC<ClientsPageProps> = ({ clients, setClients, savedQuotations, onViewQuotations }) => {
    ...
    return (
        <div className="space-y-8">
            <div className="bg-slate-800 shadow-2xl shadow-slate-950/50 rounded-lg p-6">
                <h2 className="text-2xl font-bold text-white mb-4">{isEditing ? 'Edit Client' : 'Add New Client'}</h2>
                <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="lg:col-span-1">
                        <label htmlFor="clientPhone" className="block text-sm font-medium text-slate-300 mb-1">Phone Number</label>
                        <input 
                            id="clientPhone" 
                            type="text" 
                            value={phone} 
            ...
            <div className="bg-slate-800 shadow-2xl shadow-slate-950/50 rounded-lg">
                <div className="p-6 flex flex-col md:flex-row md:justify-between md:items-center gap-4 border-b border-slate-700">
                    <h2 className="text-xl font-bold text-white">Client List ({filteredClients.length})</h2>
                    <div className="relative w-full md:max-w-xs">
                        ...
                        <input
                            ...
                            className="w-full bg-slate-700/50 border border-slate-600 rounded-lg shadow-sm pl-10 pr-4 py-2 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                    </div>
                </div>
                        ...
                        <tbody className="bg-slate-800 divide-y divide-slate-700">
                            {filteredClients.length > 0 ? (
                                filteredClients.map(client => (
                                     <tr key={client.id} className="hover:bg-slate-700/40">
                                         <td className="px-4 py-3 whitespace-nowrap text-sm text-white">{client.name}</td>
                                         <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-300">{client.phone}</td>
                                         <td className="px-4 py-3 text-sm text-slate-300 truncate max-w-xs">{client.address}</td>
                                         <td className="px-4 py-3 whitespace-nowrap text-sm text-right space-x-2">
                                            <button onClick={() => onViewQuotations(client.name)} className="text-slate-400 hover:text-green-400 transition-colors p-1" aria-label="View Quotations" title="View Quotations"><ViewListIcon className="w-5 h-5"/></button>
                                            <button onClick={() => handleEdit(client)} className="text-slate-400 hover:text-blue-400 transition-colors p-1" aria-label="Edit Client" title="Edit Client"><EditIcon className="w-5 h-5"/></button>
                                            <button onClick={() => handleDelete(client)} className="text-slate-400 hover:text-red-500 transition-colors p-1" aria-label="Delete Client" title="Delete Client"><TrashIcon className="w-5 h-5"/></button>
                                         </td>
                                     </tr>
                                 ))
                            ) : (
