...
const EditableDatalistInput: React.FC<{
    ...
    listId: string;
    className: string;
}> = ({ value, onChange, onAddOption, options, listId, className }) => {
...
export const QuotationTable: React.FC<QuotationTableProps> = ({
    ...
    const sharedThClass = `px-4 py-3 text-left text-xs font-medium uppercase tracking-wider select-none ${
        isDarkTheme ? 'text-slate-300' : 'text-gray-700'
    }`;
    const tableClass = `min-w-full table-fixed ${
        isDarkTheme ? 'divide-y divide-slate-700' : 'divide-y divide-gray-200'
    }`;
    const theadClass = isDarkTheme ? 'bg-slate-800/50' : 'bg-gray-50';
    const borderClass = isDarkTheme ? 'border-slate-700' : 'border-gray-200';
    
    const sharedInputClass = `w-full border rounded px-2 py-1 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${isDarkTheme
            ...
            <table className={tableClass}>
                ...
                <thead className={theadClass}>
                    <tr>
                        ...
                        <th scope="col" className={`${sharedThClass} border-l ${borderClass}`}>
                             <div className="flex items-center gap-2">
                                <span>Template</span>
                                <button onClick={() => setActivePage('templates')} className={`focus:outline-none focus:ring-2 focus:ring-blue-500/50 rounded-full p-0.5 ${isDarkTheme ? 'text-slate-400 hover:text-blue-400' : 'text-gray-500 hover:text-blue-500'}`} aria-label="Go to templates page">
                        ...
                        <th scope="col" className={`${sharedThClass}`}>Fullness (B)<Resizer onMouseDown={handleMouseDown(6)} /></th>
                         <th scope="col" className={`${sharedThClass} border-l ${borderClass}`}>
                             <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setActivePage('products')} className={`focus:outline-none focus:ring-2 focus:ring-blue-500/50 rounded-full p-0.5 ${isDarkTheme ? 'text-slate-400 hover:text-blue-400' : 'text-gray-500 hover:text-blue-500'}`} aria-label="Go to products page">
                                    <LinkIcon className="w-4 h-4" />
                                </button>
                            </div>
                        ...
                        <th scope="col" className={`${sharedThClass} border-l ${borderClass}`}>Front Motor<Resizer onMouseDown={handleMouseDown(10)} /></th>
                        <th scope="col" className={`${sharedThClass}`}>Back Motor<Resizer onMouseDown={handleMouseDown(11)} /></th>

                        <th scope="col" className={`${sharedThClass} border-l ${borderClass}`}>Qty<Resizer onMouseDown={handleMouseDown(12)} /></th>
                ...
                <tbody className={`${isDarkTheme ? 'bg-slate-800 divide-y divide-slate-700' : 'bg-white divide-y divide-gray-200'}`}>
                    {items.map((item, index) => (
                        <tr 
                            key={item.id} 
                            className={`transition-colors ${isDarkTheme ? 'hover:bg-slate-700/40' : 'hover:bg-gray-50'}`}
                            draggable
                        ...
                        >
                            <td className={`px-4 py-2 text-center text-sm ${isDarkTheme ? 'text-slate-400' : 'text-gray-600'}`}>
                            ...
                            {/* Unified Layer */}
                            <td className={`px-4 py-2 border-l ${borderClass}`}><SelectInput value={item.templateId} onChange={(e) => onItemChange(item.id, 'templateId', e.target.value)} options={templateOptions} className={sharedSelectClass} /></td>
                            ...
                            <td className={`px-4 py-2 border-l ${borderClass}`}><SelectInput value={item.fabricId} onChange={(e) => onItemChange(item.id, 'fabricId', e.target.value)} options={filteredOptions.fabric} className={sharedSelectClass} /></td>
                            <td className="px-4 py-2"><SelectInput value={item.sheerId} onChange={(e) => onItemChange(item.id, 'sheerId', e.target.value)} options={filteredOptions.sheer} className={sharedSelectClass} /></td>
                            <td className="px-4 py-2"><SelectInput value={item.liningId} onChange={(e) => onItemChange(item.id, 'liningId', e.target.value)} options={filteredOptions.lining} className={sharedSelectClass} /></td>
                            
                            {/* Motors & Accessories */}
                            <td className={`px-4 py-2 border-l ${borderClass}`}><SelectInput value={item.frontMotorId} onChange={(e) => onItemChange(item.id, 'frontMotorId', e.target.value)} options={filteredOptions.motor} className={sharedSelectClass} /></td>
                            <td className="px-4 py-2"><SelectInput value={item.backMotorId} onChange={(e) => onItemChange(item.id, 'backMotorId', e.target.value)} options={filteredOptions.motor} className={sharedSelectClass} /></td>

                            {/* Totals */}
                            <td className={`px-4 py-2 border-l ${borderClass}`}><input type="number" value={item.quantity} onChange={(e) => handleNumericChange(item.id, 'quantity', e.target.value)} className={sharedNumericInputClass} min="1" step="1" /></td>
                            <td className={`px-4 py-2 text-right text-sm font-semibold ${isDarkTheme ? 'text-green-300' : 'text-green-600'}`}>
                                {calculateAmount(item).toLocaleString('en-US', { style: 'currency', currency: 'BHD' })}
                            </td>
                            <td className="px-4 py-2 text-center">
                                <div className="flex items-center justify-center gap-1">
                                    <button onClick={() => onCopyItem(item.id)} className={`transition-colors p-1 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500/50 ${isDarkTheme ? 'text-slate-500 hover:text-blue-400' : 'text-gray-500 hover:text-blue-500'}`} title="Copy Item">
                                        <CopyIcon className="w-5 h-5" />
                                    </button>
                                    <button onClick={() => onRemoveItem(item.id)} className={`transition-colors p-1 rounded-full focus:outline-none focus:ring-2 focus:ring-red-500/50 ${isDarkTheme ? 'text-slate-500 hover:text-red-500' : 'text-gray-500 hover:text-red-500'}`} title="Remove Item">
