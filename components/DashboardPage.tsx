...
export const DashboardPage: React.FC<DashboardPageProps> = ({ savedQuotations, clients, setActivePage }) => {
    ...
    if (savedQuotations.length === 0) {
        return (
            <div className="text-center py-16 bg-slate-800 rounded-lg shadow-xl">
                ...
                <h3 className="mt-2 text-lg font-medium text-white">Welcome to your Dashboard</h3>
                <p className="mt-1 text-sm text-slate-400">Save your first quotation to see your sales pipeline and business insights here.</p>
                 <button
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    <div className="bg-slate-800 p-6 rounded-lg shadow-2xl shadow-slate-950/50">
                     ...
                     <div className="bg-slate-800 p-6 rounded-lg shadow-2xl shadow-slate-950/50">
                        <h2 className="flex items-center gap-3 text-xl font-bold text-white mb-4"><UsersIcon className="w-6 h-6 text-slate-400"/>Top Clients (by Approved Revenue)</h2>
            ...
            {/* Recent Activity */}
            <div className="bg-slate-800 p-6 rounded-lg shadow-2xl shadow-slate-950/50">
                <h2 className="flex items-center gap-3 text-xl font-bold text-white mb-4"><ClockIcon className="w-6 h-6 text-slate-400"/>Recent Activity</h2>
                <ul className="divide-y divide-slate-700/50">
                    {recentQuotations.map(q => (
                        <li key={q.id} className="flex justify-between items-center py-3">
                            <div>
                                <p className="font-medium text-white">{q.name}</p>
                                <p className="text-xs text-slate-400">{q.clientDetails.name} &middot; {new Date(q.savedAt).toLocaleDateString()}</p>
                            </div>
                            <div className="flex items-center gap-4">
                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-slate-700 text-slate-300`}>
                                    {q.status}
                                </span>
                                <p className="font-semibold text-blue-400 text-sm w-32 text-right">{q.grandTotal.toLocaleString('en-US', { style: 'currency', currency: 'BHD' })}</p>
                            </div>
                        </li>
                    ))}
                </ul>
