...
import type { QuotationItem, TemplateOption, ProductOption, SavedQuotation, ClientDetails, CompanyDetails, AppSettings, AppDataBackup, SelectOption, DetailedCosts, Client, QuotationVersion, DeliveryEstimateTier, QuotationStatus, StatusHistoryEntry, JobCard, JobCardStatus, DeliveryNote } from './types';
...
const QuotationControls: React.FC<QuotationControlsProps> = ({
    ...
    expirationDate, setExpirationDate, isDarkTheme = true
}) => (
    <div className={`shadow-2xl rounded-lg p-6 space-y-6 ${isDarkTheme
        ? 'bg-slate-800 shadow-slate-950/50'
        : 'bg-white shadow-gray-200/50'
        }`}>
        <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
            <div className='flex-grow'>
                <label htmlFor="quotationName" className={`block text-sm font-medium mb-1 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>Quotation Name (Optional)</label>
                <input
                    id="quotationName"
                    type="text"
                    value={quotationName}
                    ...
                    className={`w-full max-w-md border rounded-md shadow-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${isDarkTheme
                        ? 'bg-slate-700/50 border-slate-600 text-white placeholder-slate-400'
                        : 'bg-white border-gray-300 text-gray-900 placeholder-gray-500'
                        }`} />
            </div>
            <div className="flex flex-wrap gap-2">
                <button onClick={handleNewQuotation} className={`px-4 py-2 text-sm text-white font-semibold rounded-lg shadow-md ${isDarkTheme
                    ? 'bg-slate-600 hover:bg-slate-700'
                    : 'bg-gray-500 hover:bg-gray-600'
                    }`}>New Quotation</button>
                <button onClick={handlePreview} className={`px-4 py-2 text-sm text-white font-semibold rounded-lg shadow-md ${isDarkTheme
                    ? 'bg-purple-600 hover:bg-purple-700'
                    : 'bg-purple-500 hover:bg-purple-600'
                    }`}>Preview</button>
                <button onClick={handleSaveQuotation} className={`px-4 py-2 text-sm text-white font-semibold rounded-lg shadow-md ${isDarkTheme
                    ? 'bg-blue-600 hover:bg-blue-700'
                    : 'bg-blue-500 hover:bg-blue-600'
                    }`}>{currentQuotationId ? 'Update Quotation' : 'Save Quotation'}</button>
            </div>
        </div>
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 border-t pt-6 ${isDarkTheme ? 'border-slate-700' : 'border-gray-200'}`}>
            <div>
                <label htmlFor="clientName" className={`block text-sm font-medium mb-1 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>Client Name</label>
                <input
                    id="clientName"
                    list="client-names"
                    type="text"
                    ...
                    autoComplete="name"
                    className={`w-full border rounded-md shadow-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${isDarkTheme
                        ? 'bg-slate-700/50 border-slate-600 text-white placeholder-slate-400'
                        : 'bg-white border-gray-300 text-gray-900 placeholder-gray-500'
                        }`}
            ...
            <div>
                <label htmlFor="clientPhone" className={`block text-sm font-medium mb-1 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>Phone</label>
                <input
                    id="clientPhone"
                    type="tel"
            ...
            <div>
                <label htmlFor="quotationNumberDisplay" className={`block text-sm font-medium mb-1 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>Quotation #</label>
                <input id="quotationNumberDisplay" type="text" value={currentQuotationId ? savedQuotations.find(q => q.id === currentQuotationId)?.quotationNumber : quotationNumber} readOnly className={`w-full border rounded-md px-3 py-2 cursor-not-allowed ${isDarkTheme
                    ? 'bg-slate-700/80 border-slate-600 text-slate-300'
                    : 'bg-gray-100 border-gray-300 text-gray-500'
                    }`} />
            </div>
            <div>
                <label htmlFor="quotationDate" className={`block text-sm font-medium mb-1 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>Date</label>
                <input id="quotationDate" type="date" value={quotationDate} onChange={e => setQuotationDate(e.target.value)} className={`w-full border rounded-md shadow-sm px-3 py-2 ${isDarkTheme
                    ? 'bg-slate-700/50 border-slate-600 text-white'
                    : 'bg-white border-gray-300 text-gray-900'
                    }`} />
            </div>
            <div className="md:col-span-2">
                <label htmlFor="clientAddress" className={`block text-sm font-medium mb-1 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>Address</label>
                <input
                    id="clientAddress"
                    type="text"
...
interface QuotationTotalsProps {
    ...
    setFixedDiscountAmount: (value: number) => void;
...
const QuotationTotals: React.FC<QuotationTotalsProps> = ({
    ...
    fixedDiscountAmount, setFixedDiscountAmount, discountType, setDiscountType,
    discountAmount, taxPercent, setTaxPercent, grandTotal, isDarkTheme = true
}) => (
    <div className="flex justify-between items-start gap-4">
        ...
        <div className={`w-full max-w-sm shadow-2xl rounded-lg p-4 space-y-2 text-sm ${isDarkTheme
            ? 'bg-slate-800 shadow-slate-950/50'
            : 'bg-white shadow-gray-200/50'
            }`}>
            <div className="flex justify-between">
                <span className={isDarkTheme ? 'text-slate-400' : 'text-gray-600'}>Subtotal:</span>
                <span className={`font-medium ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>{subtotal.toLocaleString('en-US', { style: 'currency', currency: 'BHD' })}</span>
            </div>

            {/* Discount Controls */}
            <div className="space-y-2 py-2 border-t border-b border-slate-600">
                <div className="flex justify-between items-center">
                    <span className={isDarkTheme ? 'text-slate-400' : 'text-gray-600'}>Discount Type:</span>
                    <select
                        value={discountType}
                        onChange={e => setDiscountType(e.target.value as 'percentage' | 'fixed')}
                        className={`border rounded-md px-2 py-1 text-sm ${isDarkTheme
                            ? 'bg-slate-700/50 border-slate-600 text-white'
                            : 'bg-white border-gray-300 text-gray-900'
                            }`}
                    >
                ...
                <div className="flex justify-between items-center">
                    ...
                    <input
                        type="number"
                        value={discountType === 'percentage' ? discountPercent : fixedDiscountAmount}
                        onChange={e => {
                            const value = Number(e.target.value) || 0;
                            if (discountType === 'percentage') {
                                setDiscountPercent(Math.max(0, Math.min(100, value)));
                            } else {
                                setFixedDiscountAmount(Math.max(0, value));
                            }
                        }}
                        min="0"
                        max={discountType === 'percentage' ? 100 : undefined}
                        step={discountType === 'percentage' ? 0.1 : 0.01}
                        className={`w-24 border rounded-md px-2 py-1 text-right ${isDarkTheme
                            ? 'bg-slate-700/50 border-slate-600 text-white'
                            : 'bg-white border-gray-300 text-gray-900'
                            }`}
                    />
            ...
            {discountAmount > 0 && (
                <div className="flex justify-between">
                    <span className={isDarkTheme ? 'text-slate-400' : 'text-gray-600'}>Discount Applied:</span>
                    <span className="font-medium text-green-400">-{discountAmount.toLocaleString('en-US', { style: 'currency', currency: 'BHD' })}</span>
                </div>
            )}
            <div className="flex justify-between">
                <span className={isDarkTheme ? 'text-slate-400' : 'text-gray-600'}>Taxable Amount:</span>
                <span className={`font-medium ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>{(subtotal - discountAmount).toLocaleString('en-US', { style: 'currency', currency: 'BHD' })}</span>
            </div>
            <div className="flex justify-between items-center">
                <label htmlFor="taxPercent" className={isDarkTheme ? 'text-slate-400' : 'text-gray-600'}>Tax (%):</label>
                <input id="taxPercent" type="number" value={taxPercent} onChange={e => setTaxPercent(Number(e.target.value) || 0)} className={`w-20 border rounded-md px-2 py-1 text-right ${isDarkTheme
                    ? 'bg-slate-700/50 border-slate-600 text-white'
                    : 'bg-white border-gray-300 text-gray-900'
                    }`} />
            </div>
            <div className={`!mt-4 pt-2 border-t font-bold text-lg flex justify-between ${isDarkTheme
...
export const App: React.FC = () => {
    ...
    const [isDarkTheme, setIsDarkTheme] = usePersistentState('isDarkTheme', false);
    ...
    const [settings, setSettings] = usePersistentState<AppSettings>('appSettings', {
        ...
        defaultExpirationDays: 30, // Default 30 days expiration
    ...
    const [jobCards, setJobCards] = usePersistentState<JobCard[]>('jobCards', []);
    const [deliveryNotes, setDeliveryNotes] = usePersistentState<DeliveryNote[]>('deliveryNotes', []);
    ...
    const [fixedDiscountAmount, setFixedDiscountAmount] = usePersistentState('currentDiscountAmount', 0);
    const [discountType, setDiscountType] = usePersistentState<'percentage' | 'fixed'>('currentDiscountType', 'percentage');
    ...
    // Auto-calculate expiration date based on settings
    const expirationDate = useMemo(() => {
        if (!quotationDate || settings.defaultExpirationDays <= 0) return '';
        const quotDate = new Date(quotationDate);

        // Validate the date
        if (isNaN(quotDate.getTime())) return '';

        const expDate = new Date(quotDate.getTime() + (settings.defaultExpirationDays * 24 * 60 * 60 * 1000));

        // Double-check the calculated date is valid
        if (isNaN(expDate.getTime())) return '';

        return expDate.toISOString().split('T')[0];
    }, [quotationDate, settings.defaultExpirationDays]);
    ...
    const calculateDetailedAmount = useCallback((item: QuotationItem, useDiscountedPrices: boolean = true): DetailedCosts => {
        ...
        const getActualPrice = (product?: ProductOption) => product?.price || 0;
        const getDiscountedPrice = (product?: ProductOption) => product?.discountPrice || product?.price || 0;
        const getPrice = useDiscountedPrices ? getDiscountedPrice : getActualPrice;
    ...
    const calculateAmount = useCallback((item: QuotationItem): number => {
        return calculateDetailedAmount(item, true).total; // Use discounted prices for display
    }, [calculateDetailedAmount]);

    const calculateActualAmount = useCallback((item: QuotationItem): number => {
        return calculateDetailedAmount(item, false).total; // Use actual prices for subtotal
    }, [calculateDetailedAmount]);
    ...
    const { subtotal, discountAmount, taxAmount, grandTotal } = useMemo(() => {
        ...
        let discount = 0;
        if (discountType === 'percentage') {
            discount = (actualTotal * discountPercent) / 100;
        } else {
            discount = Math.min(fixedDiscountAmount, actualTotal); // Can't discount more than total
        }

        const subAfterDiscount = actualTotal - discount;
        const tax = (subAfterDiscount * taxPercent) / 100;
        const grand = subAfterDiscount + tax;
        return { subtotal: actualTotal, discountAmount: discount, taxAmount: tax, grandTotal: grand };
    }, [items, taxPercent, discountPercent, fixedDiscountAmount, discountType, calculateActualAmount]);
    ...
    const generateJobNumber = useCallback(() => {
        ...
        const existingJobsThisYear = jobCards.filter(jc => jc.jobNumber.startsWith(`JOB-${year}-`));
        const nextNumber = existingJobsThisYear.length + 1;
        return `JOB-${year}-${String(nextNumber).padStart(4, '0')}`;
    }, [jobCards]);

    const onCreateJobCard = useCallback((quotationId: string) => {
        ...
        const newJobCard: JobCard = {
            id: `jobcard-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
            jobNumber: generateJobNumber(),
            quotationId: quotation.id,
            quotationNumber: quotation.quotationNumber,
            clientDetails: quotation.clientDetails,
            items: quotation.items,
            status: 'Created' as JobCardStatus,
        ...
        alert(`Job Card ${newJobCard.jobNumber} created successfully!`);
    }, [savedQuotations, generateJobNumber]);

    const onUpdateJobCard = useCallback((updatedJobCard: JobCard) => {
        setJobCards(prev => prev.map(jc => jc.id === updatedJobCard.id ? updatedJobCard : jc));
    }, []);

    const onDeleteJobCard = useCallback((jobCardId: string) => {
        const jobCard = jobCards.find(jc => jc.id === jobCardId);
        if (jobCard && window.confirm(`Are you sure you want to delete job card ${jobCard.jobNumber}?`)) {
            setJobCards(prev => prev.filter(jc => jc.id !== jobCardId));
        }
    }, [jobCards]);

    // Delivery Note handlers
    const generateDeliveryNumber = useCallback((): string => {
        const year = new Date().getFullYear();
    ...
    const onCreateDeliveryNote = useCallback((deliveryNoteData: Omit<DeliveryNote, 'id' | 'createdAt' | 'updatedAt'>) => {
        ...
        alert(`Delivery Note ${newDeliveryNote.deliveryNumber} created successfully!`);
    }, []);

    const onUpdateDeliveryNote = useCallback((updatedDeliveryNote: DeliveryNote) => {
        setDeliveryNotes(prev => prev.map(dn => dn.id === updatedDeliveryNote.id ? updatedDeliveryNote : dn));
    }, []);

    const onDeleteDeliveryNote = useCallback((deliveryNoteId: string) => {
        const deliveryNote = deliveryNotes.find(dn => dn.id === deliveryNoteId);
        if (deliveryNote && window.confirm(`Are you sure you want to delete delivery note ${deliveryNote.deliveryNumber}?`)) {
            setDeliveryNotes(prev => prev.filter(dn => dn.id !== deliveryNoteId));
        }
    }, [deliveryNotes]);

    const onExport = () => {
        const backup: AppDataBackup = { products: productOptions, templates: templateOptions, savedQuotations, settings, clients, jobCards, deliveryNotes };
    ...
    const onLoadQuotation = (id: string) => {
        ...
        if (quote) {
            ...
            // Validate and set quotation date
            const loadedDate = quote.quotationDate;
            if (loadedDate && !isNaN(new Date(loadedDate).getTime())) {
                setQuotationDate(loadedDate);
            } else {
    ...
    const renderPage = () => {
        switch (activePage) {
            case 'dashboard': return <DashboardPage savedQuotations={savedQuotations.filter(q => !q.deletedAt)} clients={clients} setActivePage={setActivePage} isDarkTheme={isDarkTheme} />;
            case 'products': return <ProductsPage productOptions={productOptions} setProductOptions={setProductOptions} isDarkTheme={isDarkTheme} />;
            case 'templates': return <TemplatesPage templateOptions={templateOptions} setTemplateOptions={setTemplateOptions} productOptions={productOptions} settings={settings} isDarkTheme={isDarkTheme} />;
            case 'saved-quotations': return <SavedQuotationsPage quotations={savedQuotations} onLoad={onLoadQuotation} onDelete={onDeleteQuotation} onCopy={onCopyQuotation} onPreview={onPreviewQuotation} onRestore={onRestoreQuotation} onDeletePermanently={onDeletePermanently} onRestoreVersion={handleRestoreVersion} onPreviewVersion={handlePreviewVersion} clientFilter={clientFilter} onClearFilter={() => setClientFilter(null)} onStatusUpdate={handleStatusUpdate} isDarkTheme={isDarkTheme} />;
            case 'job-cards': return <JobCardsPage jobCards={jobCards} savedQuotations={savedQuotations} onCreateJobCard={onCreateJobCard} onUpdateJobCard={onUpdateJobCard} onDeleteJobCard={onDeleteJobCard} isDarkTheme={isDarkTheme} />;
            case 'delivery-notes': return <DeliveryNotesPage deliveryNotes={deliveryNotes} jobCards={jobCards} onCreateDeliveryNote={onCreateDeliveryNote} onUpdateDeliveryNote={onUpdateDeliveryNote} onDeleteDeliveryNote={onDeleteDeliveryNote} isDarkTheme={isDarkTheme} />;
            case 'settings': return <SettingsPage settings={settings} onSettingsChange={onSettingsChange} onExport={onExport} onImport={onImport} onReset={onReset} onResetColumnWidths={onResetColumnWidths} isDarkTheme={isDarkTheme} />;
            case 'reports': return <ReportsPage savedQuotations={savedQuotations.filter(q => !q.deletedAt)} productOptions={productOptions} templateOptions={templateOptions} calculateDetailedAmount={calculateDetailedAmount} isDarkTheme={isDarkTheme} />;
            case 'clients': return <ClientsPage clients={clients} setClients={setClients} savedQuotations={savedQuotations} onViewQuotations={onViewClientQuotations} isDarkTheme={isDarkTheme} />;
            case 'quotation':
            default:
                return (
                    <div className="space-y-6">
                        ...
                        <QuotationTable
                            ...
                            calculateAmount={calculateActualAmount}
                        ...
                        <QuotationTotals
                            ...
                            setFixedDiscountAmount={setFixedDiscountAmount}
    ...
    return (
        <div className={`min-h-screen font-sans transition-all duration-300 ${isSidebarCollapsed ? 'lg:pl-20' : 'lg:pl-64'}`}>
            <Sidebar activePage={activePage} setActivePage={setActivePage} isMobileOpen={isSidebarMobileOpen} isCollapsed={isSidebarCollapsed} setCollapsed={setSidebarCollapsed} isDarkTheme={isDarkTheme} setIsDarkTheme={setIsDarkTheme} />

            <header className={`sticky top-0 z-30 ${isDarkTheme ? 'bg-slate-900/70' : 'bg-white/70'} backdrop-blur-sm border-b ${isDarkTheme ? 'border-slate-700/50' : 'border-gray-200'} lg:hidden`}>
                <div className="mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-16">
                        ...
                        <button onClick={() => setSidebarMobileOpen(!isSidebarMobileOpen)} className={`inline-flex items-center justify-center p-2 rounded-md ${isDarkTheme ? 'text-slate-400 hover:text-white hover:bg-slate-700' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100'} focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500`} aria-expanded={isSidebarMobileOpen}><span className="sr-only">Open main menu</span><MenuIcon className="h-6 w-6" /></button>
                    </div>
                </div>
            </header>
